...
struct someCar {
    ...
    var trunkVolume : Double {
        willSet {
            if (trunkState == .empty) && (trunkVolume > 0) && (trunkVolume != 0) && (newValue < trunkVolume) {
                ...
                print ("\(brand)\(model) багажник свободен: \(space)")
            } else { print("Ошибка ввода или \(brand)\(model) багажник переполнен.")}
        }
    }
    var engineState : engineState {
        willSet {
            ...
            } else {print("\(brand)\(model) двигатель отключен")}
        }
    }
    var windowState : windowState {
        willSet {
            if newValue == .open {
                print("\(brand)\(model) окно открыто")
            } else { print("\(brand)\(model) окно закрыто") }
        }
    }
    var trunkState : trunkState
    mutating func emptyTrunck() {
        self.trunkState = .empty
        print ("\(brand)\(model) багажник переполнен")
    }
}
...
var car1 = someCar(brand: "BMW", model: "x5", color: "clear", release: 2016, trunkVolume: 580.0 , engineState: .stop, windowState: .open, trunkState: .empty)
...
print ("Информация о первом автомобиле: модель и марка: \(car1.brand)\(car1.model), релиз: \(car1.release), цвет: \(String(describing: car1.color)), Свободно в багажнике: \(car1.trunkVolume), двигатель \(car1.engineState)")
